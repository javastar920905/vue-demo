...
<script>
	...
	export default{
		...
		created () {
			...
			if (hash !== '') {
				this.isHot = true
				//不发起请求,目前写死 this.fetchData('hot', hash.substring(1))
				this.fetchLocalData('hot', hash.substring(1));
				this.isActive = false
			} else {
				//this.fetchData(type)
				this.fetchLocalData(type);
			}
			// head的修改
		...
		methods: {
			...
			fetchLocalData: function (type) {
				let vm = this
				let url, params
				let baseUrl = vm.$store.state.comm.apiUrl
				vm.$store.commit('isLoading', true)
				if (type === 'hot') {
					url ='static/image.json'
				}
				else if (type === 'fresh') {
					url ='static/image.json'
					//url = baseUrl + 'sharedapi/listOneMinuteShared'
